...
def run_graphviz(dot_string: str, out_image_file: str, renderer: str = "dot") -> None:
    ...
    try:
        ...
        p.wait(1000)
        os.remove(tmp_name)
        os.rename(tmp_name + "." + file_type, out_image_file)
    except:
        raise Exception("graphviz dot failed")


# TODO check if _escape_latex_general should come later
...
# TODO check if _escape_latex_general should come later ( \% --> \backslash{}% )
...
def run_latex(tex: str, out_file: str, include_files: list = []) -> None:
    ...
    copyfile(join(d, "main." + ext), out_file)
    rmtree(d)
