...
cdef inline dict matching(Category cat_a, Category cat_b):# -> Optional[Dict[str, Tuple[bool, str]]]:
...
cdef class Atomic(Category):
    ...
    def __cinit__(self, str label, str feature=None):
        ...
        cdef bint is_basic_punc = (label is _comma_str) or (label is _semicolumn_str) or \
                                  (label is _column_str) or (label is _dot_str)
        self.is_conj_atom = (label is _conj_str) or is_basic_punc
        self.is_punc = (label is _LRB_str) or (label is _RRB_str) or \
                       (label is _LQU_str) or (label is _RQU_str) or is_basic_punc
        self.is_left_adj_cat = False
        self.is_right_adj_cat = False
        self.is_adj_cat = False
        self.is_np = label is _NP_str
        self.is_pp = label is _PP_str
        self.is_n = label is _N_str
        self.is_nominal = self.is_np or self.is_n
        self.is_s = label is _S_str
    ...
    def __eq__(self, other):
        if isinstance(other, Atomic):
            return (self.label is other.label) and (self.feature is other.feature)
        else:
            return False
    ...
    cdef bint equals(self, Category y):
        if not y.is_atomic:
            return False
        cdef Atomic z = y
        return (z.label is self.label) and (z.feature is self.feature)

    cdef bint equals_featureless(self, Category y):
        if not y.is_atomic:
            return False
        cdef Atomic z = y
        return z.label is self.label
